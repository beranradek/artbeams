...
<#macro inputText field label type="text" required=true size="64" rowClass="form-group row" labelAsPlaceholder=false>
    <div class="${rowClass}">
        <#if labelAsPlaceholder>
            <input type="${type}" class="form-control<#if field.validationMessages?has_content> is-invalid</#if>" id="${field.elementId}" name="${field.name}" value="${field.value!}" size="${size}"<#if required> required="required"</#if> placeholder="${label}"/>
        <#else>

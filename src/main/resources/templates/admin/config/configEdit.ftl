...
<h1><#if fields.originalKey.value?has_content>Edit Config Entry<#else>New Config Entry</#if></h1>

<form action="/admin/config/save" method="post" class="form-horizontal">
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-6">
      <textarea name="${fields.entryValue.name}" id="${fields.entryValue.elementId}" rows="4" class="form-control" required>${fields.entryValue.value!}</textarea>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-6">
      ...
      <a href="/admin/config" class="btn btn-secondary">Cancel</a>
    </div>
  </div>
</form>
</@layout.page>

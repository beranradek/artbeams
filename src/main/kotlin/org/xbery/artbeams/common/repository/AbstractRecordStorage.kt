...
internal interface AbstractRecordStorage<E, R : Record> {
     ...
     */
    fun <T> updateBy(entity: E, whereField: Field<T>, fieldValue: T, entityToRecord: RecordUnmapper<E, R>): Int = dsl
        .update(table)
        .set(entityToRecord.unmap(entity))
     ...
     * @return number of affected records
     */
    fun <T> updateBy(entity: E, whereField: Field<T>, fieldValue: T): Int = dsl
        .update(table)
        .set(dsl.newRecord(table, entity))
        .where(whereField.eq(fieldValue))
        .execute()
     ...
     */
    fun <T> deleteBy(field: Field<T>, fieldValue: T): Int = dsl
        .deleteFrom(table)

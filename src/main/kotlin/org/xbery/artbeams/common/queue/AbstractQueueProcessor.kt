...
/**
    ...
    protected constructor(
        ...
        protected open fun createTaskState(): QueueProcessingTaskState = QueueProcessingTaskState()
        ...
        protected open fun getFinishExpiration(entry: E): Instant = Instant.now(clock).plus(preserveAfterFinished)
        ...
        protected open fun getNextRetry(entry: E): Instant? = retry.calculateNextRetry(
            Instant.now(clock),
            entry.entered.time,
            entry.attempts
        )

        protected open fun getFailureExpiration(entry: E): Instant = Instant.now(clock).plus(preserveAfterGivenUp)

        protected open fun isPermanentError(error: Throwable?): Boolean = false
         ...
         */
        protected open fun discontinueTaskNow(state: QueueProcessingTaskState): Boolean = if (state.processed >= maxPerRun) {
         ...
         */
        protected open fun isPausingException(error: Throwable?): Boolean = false

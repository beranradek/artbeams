...
class ConsentRepository(
     ...
     */
    fun findByLogin(login: String): List<Consent> = dsl
        .selectFrom(table)
     ...
     */
    fun findByLoginAndType(login: String, consentType: ConsentType): List<Consent> = dsl
        .selectFrom(table)
        .where(CONSENTS.LOGIN.eq(login))
     ...
     */
    fun findValidConsents(login: String, consentType: ConsentType, timestamp: Instant): List<Consent> = dsl
        .selectFrom(table)
        .where(CONSENTS.LOGIN.eq(login))
        .and(CONSENTS.CONSENT_TYPE.eq(consentType.name))

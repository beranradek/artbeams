...
class OrderRepository(
    ...
    fun findOrders(): List<OrderInfo> = findOrdersWithFilter(null)
    ...
    fun findOrders(pagination: Pagination): ResultPage<OrderInfo> {
        ...
        val records = dsl
            ...
            .leftJoin(USERS)
            .on(ORDERS.CREATED_BY.eq(USERS.ID))
            .leftJoin(ORDER_ITEMS)
            .on(ORDERS.ID.eq(ORDER_ITEMS.ORDER_ID))
            .leftJoin(PRODUCTS)
    ...
    fun findOrdersByUserId(userId: String): List<OrderInfo> = findOrdersWithFilter(userId)
    ...
    private fun findOrdersWithFilter(userId: String?): List<OrderInfo> {
        ...
        val records = if (userId != null) {
            baseQuery
                .where(ORDERS.CREATED_BY.eq(userId))
                .orderBy(ORDERS.CREATED.desc())
                .fetch()
        } else {
            baseQuery
    ...
    fun updateOrderState(orderId: String, state: OrderState): Boolean = dsl
        .update(ORDERS)
